    ...
    for src, tgt in chunk:
        if is_release(nodes[src]):  # Only consider release nodes for the source
            ...
            if not is_release(nodes[tgt]):  # If the target is a software node
                ...
                if tgt in rel_to_soft():  # Check if the software has associated releases
                    for release in rel_to_soft()[tgt]:  # Iterate through all releases of the software
...
class DepGraph:
    ...
    def time_ranges(self, software_to_release: dict):
        ...
        for software, releases in software_to_release.items():
            for i, (nid, timestamp) in enumerate(releases):
                if i < len(releases) - 1:
                    next_timestamp = releases[i + 1][1]
                else:
                    next_timestamp = float('inf')  # Open-ended range for the last release
                timestamp_ranges[nid] = (timestamp, next_timestamp)
    ...
    def filter_edges(self, software_to_releases):
        ...
        for src, tgt, attr in self.edges:
            if attr['label'] == 'dependency':
                # release to software
                filter_edges.append((src, tgt))
            elif attr['label'] == 'relationship_AR':
                # release to software
                filter_edges.append((tgt, src))
            else:
                continue
        return filter_edges
    ...
    def dep_graph_build_parallel(self, filter_edges, time_ranges):
        ...
        chunk_size = (len(filter_edges) + num_processes - 1) // num_processes  # Ceiling division
        edge_chunks = [filter_edges[i:i + chunk_size] for i in range(0, len(filter_edges), chunk_size)]
